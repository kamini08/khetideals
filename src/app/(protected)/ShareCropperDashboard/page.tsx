                ...
                ongoingContracts.map(
                  (contract: any, index: React.Key | null | undefined) => (
                    <div className="contract-card" key={index}>
                      <h3 className="mb-4">LandHolder: {contract.landholder.name}</h3>
                      <p className="mb-4">Crop Type: {contract.landDetails.cropToGrow}</p>
                      <p className="mb-4">
                        Soil Type: {contract.landDetails.soilType} 
                      </p>
                      <p className="mb-4">
                        Land Area: {contract.landDetails.areaOfLand} 
                      </p>
                      <p className="mb-4">
                        Price: ${contract.financialDetails.totalCost}
                      </p>
                      <p className="mb-4">Status: {contract.contract2Status}</p>
                      <Link href={`/contract/${contract.contract2Id}`}>
                        <button className="btn m-4 border-2 border-green-800 rounded-xl mx-auto block text-wrap">
                          View Details
                        </button>
                      </Link>
                      <button
                        className="btn m-4 border-2 border-green-800 rounded-xl mx-auto block text-wrap"
