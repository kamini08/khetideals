...
export default async function generateContract2PDF(contract2: {
    landholder: {
      ...
      doc.text("Land Sharing Agreement", pageWidth / 2 + margin.left, margin.top, { align: "center" });
      ...
      doc.text(`Address: ${contract2.landholder.address}`, margin.left, margin.top + 60);
      doc.text(`Phone: ${contract2.landholder.phoneNumber}`, margin.left, margin.top + 70);
      doc.text(`Email: ${contract2.landholder.email}`, margin.left, margin.top + 80);
    
      // Sharecropper Information
      doc.setFont("helvetica", "bold");
      doc.text("Sharecropper Information", margin.left, margin.top + 100);
      doc.setFont("helvetica", "normal");
      doc.text(`Name: ${contract2.sharecropper.name}`, margin.left, margin.top + 110);
      doc.text(`Address: ${contract2.sharecropper.address}`, margin.left, margin.top + 120);
      doc.text(`Phone: ${contract2.sharecropper.phoneNumber}`, margin.left, margin.top + 130);
      doc.text(`Email: ${contract2.sharecropper.email}`, margin.left, margin.top + 140);
      ...
      autoTable(doc, {
        ...
        head: [["Detail", "Amount"]],
        body: [
          ["Price per Decimal", `$${contract2.financialDetails.pricePerDecimal.toFixed(2)}`],
          ["Total Cost", `$${contract2.financialDetails.totalCost.toFixed(2)}`],
      ...
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");
      doc.text(`1. Crop Cycle: The crop cycle will start in ${contract2.cropCycle.startingMonth} and end in ${contract2.cropCycle.endingMonth}.`, margin.left, finalY2 + 30);
      doc.text(
        `2. Payment: The sharecropper will pay the landholder based on the agreed price per decimal, with the total cost amounting to $${contract2.financialDetails.totalCost.toFixed(2)}.`,
        margin.left,
        finalY2 + 40
      );
      doc.text(
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Landholder Signature: _____________________________", margin.left, finalY2 + 110);
      doc.text("Date: _____________________________", margin.left, finalY2 + 120);
      doc.text("Sharecropper Signature: _____________________________", margin.left, finalY2 + 130);
